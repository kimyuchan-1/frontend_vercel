...
export async function GET(request: Request) {
  ...
  const provinceQ = (searchParams.get("province") ?? "").trim();
  if (!provinceQ) return NextResponse.json({ error: "province is required" }, { status: 400 });
  ...
  for (const r of rows) {
    const { province, city } = tokens2(r.bjd_nm);
    if (province !== provinceQ) continue;
    if (!city) continue;
