  ...
  try {
    ...
    const content = (data ?? []).map((row: any) => {
      ...
      return {
        ...
        status: row.status,
        priority_score: row.priority_score ?? 0.0,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload = {
      ...
      location_lon,
      address,
      priority_score: Number.isFinite(Number(priority_score)) ? Number(priority_score) : 0.0,
    ...
    const result = {
      ...
      status: data.status,
      priority_score: data.priority_score ?? 0.0,
